...
def install_bootloader() -> None:
    ...
    for (profile, gens) in profiles:
        sorted_gens = sorted(gens, key=lambda x: x, reverse=True)
        latest_gen = sorted_gens[0]

        # Standalone top-level entry that auto-boots (default_entry: 1 points here)
        latest_time = datetime.datetime.fromtimestamp(
            os.stat(get_system_path(profile, latest_gen), follow_symlinks=False).st_mtime
        ).strftime("%F %H:%M:%S")
        latest_json = json.load(open(os.path.join(get_system_path(profile, latest_gen), 'boot.json'), 'r'))
        latest_spec = bootjson_to_bootspec(latest_json)
        config_file += config_entry(1, latest_spec, config('distroName'), latest_time)

        # Dropdown folder with all generations
        group_name = 'Generations' if profile == 'system' else f"Generations '{profile}'"
